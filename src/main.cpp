...
json generateSSHKey(const std::string &keyType, const std::string &keySize,
    ...
    std::string filename_base = app_dir + "/" + sanitized_email + "_" + std::to_string(timestamp);
    ...
    rc = ssh_pki_export_privkey_base64(key, passphrase.empty() ? nullptr : passphrase.c_str(),
