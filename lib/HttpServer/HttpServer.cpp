...
HttpServer::HttpServer(const int port) : server_fd(-1), port(port), running(false) {
}
...
void HttpServer::register_route(const std::string& path,
        ...
        if (!headers_complete) {
            if (const auto header_end = request_str.find("\r\n\r\n"); header_end != std::string::npos) {
                ...
                if (std::getline(stream, line)) {
                    ...
                    if (const size_t path_end = line.find(' ', method_end + 1); method_end != std::string::npos && path_end != std::string::npos) {
                        ...
                        std::cerr << "Malformed request line: " << line << std::endl;
                        return false;
                    }
                }
...
void HttpServer::process_request(const int client_socket, const HttpRequest& request) {
    if (const auto handler_it = route_handlers.find(request.path); handler_it != route_handlers.end()) {
        std::string response_body;
        std::unordered_map<std::string, std::string> response_headers;
        handler_it->second(request.method, request.headers, request.body, response_body, response_headers);
        send_response(client_socket, 200, "OK", response_headers, response_body);
        return;
    }
    if (serve_file_from_vfs(client_socket, request.path)) {
        return;
    }
    const std::string not_found_body = "<html><body><h1>404 Not Found</h1></body></html>";
...
bool HttpServer::serve_file_from_vfs(const int client_socket, const std::string& path) {
...
void HttpServer::send_response(const int client_socket,
